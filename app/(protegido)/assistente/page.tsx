...
export default function PaginaAssistente() {
  ...
  // Morning briefing
  const { data: briefing, isLoading: carregandoBriefing } = useBriefingMatinal()
  const [briefingExibido, setBriefingExibido] = React.useState(false)

  // Voice recording
  const handleTranscricao = React.useCallback((texto: string) => {
    setTextoAtual(prev => prev ? `${prev} ${texto}` : texto)
  }, [])
          ...
          <div className="flex min-h-0 flex-1 flex-col gap-8 pb-8">
            {mostrarHero ? (
              <div className="flex flex-1 flex-col items-center justify-center gap-8 text-center">
                ...
                {(carregandoBriefing || briefing) && !briefingExibido && (
                  <div className="w-full max-w-3xl">
                    <Cartao className="border-primary/20 bg-gradient-to-br from-primary/5 to-amber-500/5">
                      <CartaoConteudo className="p-5">
                        <div className="mb-3 flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-primary/10">
                              <Zap className="h-4 w-4 text-primary" />
                            </div>
                            <p className="text-sm font-semibold">Briefing Matinal</p>
                          </div>
                          <Botao
                            variant="ghost"
                            size="sm"
                            onClick={() => setBriefingExibido(true)}
                        ...
                        {carregandoBriefing ? (
                          ...
                          <div className="whitespace-pre-wrap text-left text-sm leading-relaxed text-foreground/90">
                            {briefing}
                          </div>
                        )}
                      </CartaoConteudo>
          ...
          {gravacao.isGravando && (
            <div className="mb-2 flex items-center gap-3 rounded-xl border border-destructive/50 bg-destructive/5 px-4 py-3">
              ...
              <div className="flex flex-col">
                ...
                {gravacao.isTranscrevendo && (
                  <span className="text-xs text-muted-foreground">Transcrevendo em tempo real...</span>
                )}
              </div>
              <Botao
                variant="ghost"
